...
#ifndef SAT_UTIL_BITSET_H_
#define SAT_UTIL_BITSET_H_
...
inline int LeastSignificantBitPosition64Fast(uint64 n) {
    // Note(user): Do not change the order of instructions. Other patterns
    // were tried, and the second best was:
    // return n == 0 ? 0 : __builtin_ctzll(n) which results in an
    // 2x increase of computation time.
...
class Bitset64 {
 public:
        ...
        // Resizes the Bitset64 to the given number of bits.
        // New bits are sets to 0.
        ...
        // Changes the number of bits the Bitset64 can hold and set all of
        // them to 0.
        void ClearAndResize(IndexType size) {
            ...
            // TODO(user): Ideally if a realloc happens, we don't need to copy
            // the old data...
            const size_t bit_length =
                static_cast<size_t>(BitLength64(Value(size_)));
        ...
        // Returns true if the bit at position i or the one at
        // position i ^ 1 is set.
        ...
        // If value is true, sets the bit at position i to 1,
        ...
        // Copies "other" to "this". The bitsets do not have to be of
        // the same size.
        // If "other" is smaller, high order bits are not changed. If "other" is
        // larger, its high order bits are ignored. In any case "this" is
        // not resized.
        ...
        // Same as SetContentFromBitset where "this" and "other" have
        // the same size.
        template <typename OtherIndexType>
        ...
        //
        // IMPORTANT: Because the iterator "caches" the current uint64 bucket,
        // this will probably not do what you want if Bitset64 is modified
        // while iterating.
        class Iterator {
         public:
                ...
                void Next() {
                    ...
                    // Computes the index and clear the least significant bit
                    // of current_.
                    index_ = base_index_ +
                        LeastSignificantBitPosition64(current_);
                    current_ &= current_ - 1;
                }

                // STL version of the functions above to support range-based
                // "for" loop.
        ...
        // Cryptic function!
        // This is just an optimized version of a given piece of code and has
        // probably little general use. Sets the bit at position i to th
        // e result of (other1[i] && use1) XOR (other2[i] && use2).
 ...
 private:
        ...
        // It is faster to store the end() Iterator than to recompute it
        // every time.  Note that we cannot do the same for begin().
        const Iterator end_;
...
// Specialized version of Bitset64 that allows to query the last bit set
// more efficiently.
class BitQueue64 {
 public:
        ...
        // Returns the position of the highest bit set in O(1)
        ...
        void ClearTop() {
            ...
            // Note(user): I experimented with reversing the bit order in a
            // bucket to use LeastSignificantBitPosition64() and it is only
            // slightly faster at the cost of a lower Set() speed.
            // So I prefered this version.
            top_ = BitShift64(bucket_index) +
...
class SparseBitset {
 public:
        ...
        void ClearAndResize(IntegerType size) {
            // As of 19/03/2014, experiments show that this is a
        ...
        // Tells the class that all its bits are cleared, so it can reset
        // to_clear_ to the empty vector. Note that this call is "unsafe"
        // since the fact that the class is actually all cleared is only
        // checked in debug mode.
        //
        // This is useful to iterate on the "set" positions while clearing
        // them for instance.
        // This way, after the loop, a client can call this for efficiency
        void NotifyAllClear() {
            if (DEBUG_MODE) {
